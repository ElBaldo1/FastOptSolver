...
def profile_solver_multiple_runs_housing(
    solver_cls,
    loss_name,
    alpha,
    alpha2=None,
    ...
    **kwargs
    ...
    for seed in tqdm(range(n_runs), desc=f"{solver_cls.__name__} profiling [real data]"):
        result = run_solver_on_housing(
...
def profile_solver_adaptive_housing(
    solver_cls,
    loss_name,
    alpha,
    alpha2=None,
    ...
    **kwargs
    ...
    for seed in tqdm(range(n_runs), desc=f"{solver_cls.__name__} adaptive [real data]"):
        result = run_solver_on_housing(
            solver_cls=solver_cls,
            loss_name=loss_name,
            alpha=alpha,
